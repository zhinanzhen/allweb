...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '该文件无法上传，错误原因：';
...
/*			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('该文件【"' + file.name + '"】 已在队列中.\n确认要替换它吗？');
					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) <= 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n【 "' + file.name + '"】 超过大小限制，请控制在(' + settings.fileSizeLimit + ')范围内！';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n该文件【 "' + file.name + '"】 为空，请重新上传！'; 
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n该文件【 "' + file.name + '"】类型只能为【 (' + settings.fileTypeDesc + ')】！';
						break;
				}
			}
